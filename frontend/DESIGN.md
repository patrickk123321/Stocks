...
**Homepage logo intro** (`frontend/app/components/LogoIntro.tsx`): wraps `PeloSiMark` for the homepage hero only (the header's copy of the mark stays fully static). On a first visit this tab, the rocket flies in from off-screen bottom-left through an S-curve (keyframed `x`/`y`/`rotate` on a `motion.div`, landing at `{ x: 0, y: 0 }` — its normal flex-flow position — with rotation settling at the artwork's own baked-in 28° tilt), trailing a handful of fading smoke-puff circles. Once the flight finishes, a speech bubble draws itself on (the bubble outline's `stroke-dashoffset` animates from its full length to 0 — the classic SVG "draw the line" technique — rather than fading or popping in), then "ToTheMoon" fades in inside it. The title occupies no DOM space at all until this second phase, in a layout container sized in advance so nothing around it shifts when it appears. Same session-gating/reduced-motion/mount-gating/Strict-Mode-guard conventions as `CandlestickHero.tsx` below, under its own independent `sessionStorage` key — these are two separate one-time animations, not one shared gate.

**Hero animation** (`frontend/app/components/CandlestickHero.tsx`): the homepage's one animated set piece, and the dominant visual on the page — the homepage is a single full-height two-column hero: logo/title/description/status-pill/Trades-card on the left (top-anchored title, the pill and Trades card centered together in the remaining space below), this chart filling the right column. A row of candlesticks colored by their real open/close direction (`--color-positive` green if up, `--color-destructive` red if down — alternating, not monotonic) animate in with a calm staggered entrance, ending in a much larger green "breakout" candle with a spring-overshoot snap and one soft radial-gradient glow flash. **The glow is the one deliberate glow-decoration exception in the entire UI** (see the anti-pattern note below) — restrained and warm-tinted, not neon; the breakout doesn't need a color *transition* (unlike an earlier version of this component) since it's simply rendered green like other up-candles, just bigger and springier. After the breakout settles, a smooth trend line (tracing each candle's close price, built via quadratic-Bezier midpoint smoothing, not hardcoded coordinates) fades in on top of the candles with a gradient area fill beneath it down to the baseline — **a dark brown (`#4a2c17`), also a deliberate scoped exception to Ledger tokens**, same treatment as the brand mark's colors, chosen to read clearly against both the green/red candles and the mark. Plays once per browser tab (`sessionStorage`-gated) and respects `prefers-reduced-motion` by skipping straight to the settled final state (candles at full height, trend line already visible, no glow). Mount-gated (renders nothing until a client effect resolves session/reduced-motion state) to avoid any SSR-hydration flash, and guarded with a `useRef` against React Strict Mode's dev-only double-invocation of effects — without that guard, the effect's second invocation reads back its own `sessionStorage` write from the first invocation and concludes "already played" before anything ever animates.
