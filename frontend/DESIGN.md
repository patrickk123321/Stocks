...
This document describes the visual design system of the Pelo$i frontend as it actually exists in code today. It is derived from the shipped source (`frontend/app/globals.css`, `layout.tsx`, and the components listed below), not from a plan or intention. If something here stops matching the code, the code wins — update this file.
...
**Brand mark** (`frontend/app/components/PeloSiMark.tsx`): a hand-built inline SVG — an original caricature illustration (a suited, sunglassed, blonde-haired figure riding a tilted rocket, one arm raised), satirical in the same spirit as the "Pelo$i" name, not traced or copied from any existing artwork or photo. **This is the one component in the app whose colors are not Ledger tokens** — a deliberate, scoped exception (same treatment as the glow exception below): rocket red/white, suit navy, flame orange/yellow, skin/hair/sunglasses tones are hardcoded hex local to this file, because the whole point of the mark is recognizability against a specific reference, not palette consistency. Used at two sizes from the same component — small (`size={16}`, `Header.tsx`'s logo frame) and large (`size={100}`, the homepage hero, sitting to the left of the title/description) — kept as bold, simple shape primitives rather than fine detail so one mark reads at both scales. Duplicated as static markup (same coordinates/colors) in `frontend/app/icon.svg` for the favicon, since a standalone favicon file can't reference a React component.
